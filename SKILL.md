...
get_logs(job_id, tail=100)             # 로그 조회
...
job_id로 공개 URL을 구성하여 사용자에게 링크를 제시한다.
**브라우저에서 직접 열 수 있으며, 인증 불필요.**

사용자에게 이렇게 안내한다:

> 아래 링크를 브라우저에서 열면 로그를 확인할 수 있습니다:
> - 로그: `https://nanobody-aca-api.politebay-55ff119b.westus3.azurecontainerapps.io/v1/design-jobs/<job_id>/logs/public?tail=200`
> - 상태: `https://nanobody-aca-api.politebay-55ff119b.westus3.azurecontainerapps.io/v1/design-jobs/<job_id>/status/public`
...
> **HTML artifact를 생성하지 않는다.** Claude Desktop artifact sandbox는 외부 네트워크 요청을 차단한다.
...
- **YAML 검증 실패**: chain ID 대소문자, 1-based 잔기 인덱스 확인 → Mol* 뷰어: https://molstar.org/viewer/
